...
Related: [PRODUCTION.md](PRODUCTION.md) (credentials, hosting, E2E A/B/C) · [RELEASE.md](RELEASE.md) (post-TestFlight cadence) · [XCODE_CLOUD.md](XCODE_CLOUD.md) (GM Archive from Cloud when on Xcode beta).
...
1. Build phase **Sign PrivySDK XCFramework** runs `scripts/sign-privy-xcframework.sh` after Frameworks / Resources (so Archive can populate `EXPANDED_CODE_SIGN_IDENTITY`).
2. Script injects a minimal `PrivacyInfo.xcprivacy` into each `PrivySDK.framework` slice (if missing) and `codesign`s the XCFramework (prefers Apple Distribution; on Xcode Cloud Build with an empty keychain, creates an ephemeral self-signed signer).
...
2. Confirm Peak target → Build Phases includes **Sign PrivySDK XCFramework** after Frameworks / Resources.
...
4. On Xcode Cloud, use workflow action **Archive - iOS** (not only Build) — see [XCODE_CLOUD.md](XCODE_CLOUD.md).
5. Bump `CURRENT_PROJECT_VERSION`, **Product → Archive**, export/upload as usual (README **Shipping a build**).
